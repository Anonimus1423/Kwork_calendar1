...
{
    let date = new Date(Date.UTC(obj.year, (russianMonths.findIndex(month => month === obj.month) + 1) - 1, (obj.day || 1)));
...
{
        let lastDay = dateFrom.day;
        let lastDay1 = dateTo.day;
        ...
        if(checkCalendarValidation(order))
        {
...
export function dateFormSubmit()
{
    ...
    document.getElementById('date-input-container-2').addEventListener('submit', function(event) {
        event.preventDefault();
    });
}
...
function checkCalendarValidation(order)
{
    if(order === 1)
    {
        if(getDateValues()[0] > getDateValues()[1] && (dateFrom.day !== null && dateTo.day !== null))
        {
            document.getElementById('error-message').textContent = 'Дата начала периода ппревиышает дату окончания';
            return false;
        }
        document.getElementById('error-message').textContent = '';
        return true
...
function getDateValues()
{
    ...
    const day1 = dateFrom.day;
    const day2 = dateTo.day;
    const firstDateValue = year1 + (month1 / 13) + (day1 / 356);
    const secondDateValue = year2 + (month2 / 13) + (day2 / 356);
